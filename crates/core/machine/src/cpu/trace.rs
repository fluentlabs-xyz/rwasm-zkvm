...
use sp1_core_executor::{
    events::{create_alu_lookups, AluEvent, ByteLookupEvent, ByteRecord, CpuEvent, LookupId},
...
impl CpuChip {
    ...
    fn event_to_row<F: PrimeField32>(
        ...
        *cols.op_a_access.value_mut() = event.exec_memory_list.arg1.into();
        *cols.op_b_access.value_mut() = event.exec_memory_list.arg2.into();
        *cols.op_c_access.value_mut() = event.exec_memory_list.arg3.into();

        // Populate memory accesses for a, b, and c.
        if let Some(record) = event.exec_memory_list.arg1_record {
            cols.op_a_access.populate(event.channel, record, blu_events);
        }
        if let Some((record)) = event.exec_memory_list.arg2_record {
            cols.op_b_access.populate(event.channel, record, blu_events);
        }
        if let Some((record)) = event.exec_memory_list.arg3_record {
    ...
    /*
    disable memory now because not needed now
    TODO reuse this when we start working on memory */
