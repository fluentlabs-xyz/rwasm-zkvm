...
use rwasm::mem_index::{AddressType, TypedAddress, SP_END, UNIT};
...
use sp1_stark::air::{ByteAirBuilder, SP1AirBuilder};
...
impl<F: PrimeField32> StackAddressCols<F> {
    pub fn populate(&mut self, addr: u32, output: &mut impl ByteRecord) {
        ...
        output.add_u16_range_check(addr);
...
impl<F: Field> StackAddressCols<F> {
    pub fn range_check<AB: SP1AirBuilder>(builder: &mut AB, cols: StackAddressCols<AB::Var>) {
        builder.assert_bool(cols.is_real.clone());
        ...
        builder.send_byte(
            ...
            AB::Expr::zero(),
            cols.addr_hi_8bits.clone(),
            cols.addr_low_8bits.clone(),
            cols.is_real.clone(),
        );
        //assert that the two componets are correct;
        builder.when(cols.is_real.clone()).assert_eq(
            cols.addr.clone(),
            cols.addr_hi_8bits.clone() * AB::Expr::from_canonical_u32(1 << 8)
                + cols.addr_low_8bits.clone(),
        );

        builder.assert_bool(cols.addr_hi_is_lt_stack_ub_hi.clone());
        //We first check if the addr_hi is less than the STACK UB hi

        builder.send_byte(
            AB::Expr::from_canonical_u32(ByteOpcode::LTU as u32),
            cols.addr_hi_is_lt_stack_ub_hi.clone(),
            cols.addr_hi_8bits,
        ...
        builder
            .when(cols.is_real.clone())
            .when_not(cols.addr_hi_is_lt_stack_ub_hi)
            .assert_eq(cols.addr_hi_8bits, AB::Expr::from_canonical_u8(STACK_UB_HI_8BITS));

        //LB check.
        // Note that we cannot rely on addr_hi_is_zero because when not addr_hi_is_zero might be the case it is an empty cols
