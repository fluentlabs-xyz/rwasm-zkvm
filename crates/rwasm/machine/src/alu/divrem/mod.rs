...
use rwasm_executor::{
    ...
    get_msb, get_quotient_and_remainder, is_signed_operation, ByteOpcode, ExecutionRecord, Program,
    DEFAULT_PC_INC, I32MULHU_CODE, I32MULH_CODE, UNUSED_PC,
};
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        // Use the mul table to compute c * quotient and compare it to local.c_times_quotient.
        {
            ...
            let opcode_for_upper_half = {
                let mulh = AB::Expr::from_canonical_u32(I32MULH_CODE);
                let mulhu = AB::Expr::from_canonical_u32(I32MULHU_CODE);
...
mod tests {
    ...
    fn generate_trace() {
        ...
        shard.divrem_events =
            vec![AluEvent::new(0, Opcode::I32DivU, 2, 17, 3, Opcode::I32DivU.code())];
