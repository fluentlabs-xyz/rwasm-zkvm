...
namespace rwasm_machine_sys::sr {
template <class F>
__SP1_HOSTDEV__ void event_to_row(const AluEvent& event,
                                  ShiftRightCols<decltype(F::val)>& cols) {
  ...
  array_t<uint8_t, LONG_WORD_SIZE> byte_shift_result{};
  {
    ...
    array_t<uint8_t, 8> sign_extended_b =
        event.opcode == Opcode::SRA ? u64_to_le_bytes((int64_t)(int32_t)event.b)
