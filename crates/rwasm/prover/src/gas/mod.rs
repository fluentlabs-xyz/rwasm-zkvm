...
use rwasm_executor::{estimator::RecordEstimator, RwasmAirId};
use rwasm_machine::shape::{CoreShapeConfig, CoreShapeError, Shapeable, ShardKind};
...
pub fn estimated_records<'a>(
...
) -> impl Iterator<Item = Cow<'a, EnumMap<RwasmAirId, u64>>> {
    let RecordEstimator {
        ref core_records,
    ...
    let core_records = core_records.iter().map(|shard| {
        if (shard[RwasmAirId::Global] as f64).log2().fract() < THRESHOLD {
            let mut shard = *shard;
            shard[RwasmAirId::Global] = 1 << shard[RwasmAirId::Global].ilog2();
            Cow::Owned(shard)
        } else {
            Cow::Borrowed(shard)
        }
    ...
    let precompile_records = precompile_records.iter().flat_map(|(id, shards)| {
        shards.iter().map(move |&(precompile_events, local_memory_events)| {
            Cow::Owned(EnumMap::from_iter([
                ...
                (RwasmAirId::MemoryLocal, local_memory_events),
            ]))
        })
    });
    ...
    let global_memory_records = {
        ...
        #[inline]
        fn memory_air(init_ht: u64, final_ht: u64) -> EnumMap<RwasmAirId, u64> {
            EnumMap::from_iter([
                (RwasmAirId::MemoryGlobalInit, init_ht),
                (RwasmAirId::MemoryGlobalFinalize, final_ht),
                (RwasmAirId::Global, init_ht + final_ht),
            ])
        }
...
pub fn fit_records_to_shapes<'a, F: PrimeField32>(
    ...
    records: impl IntoIterator<Item = Cow<'a, EnumMap<RwasmAirId, u64>>> + 'a,
) -> impl Iterator<Item = Result<Shape<RwasmAirId>, CoreShapeError>> + 'a {
...
struct CoreShard<'a> {
    shard_index: u32,
    record: &'a EnumMap<RwasmAirId, u64>,
}
...
impl Shapeable for CoreShard<'_> {
    ...
    fn log2_shard_size(&self) -> usize {
        self.record[RwasmAirId::Cpu].next_power_of_two().ilog2() as usize
    }
    ...
    fn precompile_heights(&self) -> impl Iterator<Item = (RwasmAirId, (usize, usize, usize))> {
        let num_local_mem_events = self.record[RwasmAirId::MemoryLocal] as usize;
